"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var AdminUserUseCase_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.AdminUserUseCase = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const user_entity_1 = require("../entity/user.entity");
let AdminUserUseCase = AdminUserUseCase_1 = class AdminUserUseCase {
    usersRepository;
    logger = new common_1.Logger(AdminUserUseCase_1.name);
    constructor(usersRepository) {
        this.usersRepository = usersRepository;
    }
    async createUser(adminCreateUserInputDto) {
        const newUser = new user_entity_1.User();
        newUser.name = adminCreateUserInputDto.name;
        newUser.role = adminCreateUserInputDto.role;
        const reuslt = await this.usersRepository.save(newUser);
        this.logger.log(`${reuslt.name} has been created !!!`);
        return reuslt;
    }
    async findAll() {
        return this.usersRepository.find();
    }
};
AdminUserUseCase = AdminUserUseCase_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(user_entity_1.User)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], AdminUserUseCase);
exports.AdminUserUseCase = AdminUserUseCase;
//# sourceMappingURL=admin-user.usecase.js.map